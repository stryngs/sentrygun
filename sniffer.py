...
def extract_probe_data(packet, nicDriver):

    ## Deal with driver offset
    notDecoded = hexstr(str(packet.notdecoded), onlyhex = 1).split(' ')
    if nicDriver.offset:
        try:
            fSig = -(256 - int(notDecoded[nicDriver.offset + 3], 16))
        except IndexError:
            fSig = ''
    else:
        fSig = ''
...
def response_sniffer(interface, nicDriver):

    while True:
        pkt = sniff(lfilter = is_valid_probe_response,
                    iface = interface,
                    count = 1,
                    store = 1,
                    timeout = 10)
        ...
        if len(pkt) > 0:
            try:
                yield extract_probe_data(pkt[0], nicDriver)
            except IndexError:
                continue
